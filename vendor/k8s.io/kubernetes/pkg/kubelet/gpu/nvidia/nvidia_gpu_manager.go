...
func (ngm *nvidiaGPUManager) Capacity() v1.ResourceList {
	gpu_multiplyer := 100
	gpus := resource.NewQuantity(int64(len(ngm.allGPUs)*gpu_multiplyer), resource.DecimalSI)
...
func (ngm *nvidiaGPUManager) AllocateGPU(pod *v1.Pod, container *v1.Container) ([]string, error) {
	...
	// Get a list of all_gpus GPUs.
	all_gpus := ngm.allGPUs // Make alls GPUs available to the container
	glog.V(5).Infof("gpus all_gpus: %v", all_gpus.List())
	if int64(all_gpus.Len()) < gpusNeeded {
		return nil, fmt.Errorf("requested number of GPUs unavailable. Requested: %d, Available: %d", gpusNeeded, all_gpus.Len())
	}
	ret := all_gpus.UnsortedList() // Assign all GPUs to the container
